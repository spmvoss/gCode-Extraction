...
def intermediates(p1, p2, z, d2 = 0.1**2):
    ...
    return [[p1[0] + i * x_spacing, p1[1] +  i * y_spacing, z]
                ...
                if "X" in items:
                    ...
                    for units in rows:
                        if "E" in units:
                            ...
                            if coordinate not in layers[key_name] and 0.05 < dist < 0.5:
                            ...
                            elif coordinate not in layers[key_name] and dist >= 0.5:
                                intermediateCoords = intermediates(coordinate, previousCoord, z=currentZ,d2 = 0.1**2)
                                layerList = layers[key_name]  # Grab the current list of coordinates
                                for coords in intermediateCoords:
                                    layerList.append(coords)  # Append with new coordinate
                                layerList.append(coordinate)    # Append with new coordinate
                                layers[key_name] = layerList    # Write back to dictionary
